...
namespace ZJY.Framework
{
    ...
    public class ResourceComponent : GameBaseComponent
    {
        ...
        protected override void OnAwake()
        {
            ...
            m_ResourceManager.AssetAutoReleaseInterval = m_ResourceAutoReleaseInterval;
            m_ResourceManager.AssetExpireTime = m_ResourceExpireTime;
        }
        ...
        {
            GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ResourceUpdateStartGameEvent>().Fill(name, downloadPath, downloadUri, currentLength, zipLength, retryCount));
        }

        private void OnResourceUpdateChanged(string name, string downloadPath, string downloadUri, int currentLength, int zipLength)
        {
            GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ResourceUpdateChangedGameEvent>().Fill(name, downloadPath, downloadUri, currentLength, zipLength));
        }

        private void OnResourceUpdateSuccess(string name, string downloadPath, string downloadUri, int length, int zipLength)
        {
            GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ResourceUpdateSuccessGameEvent>().Fill(name, downloadPath, downloadUri, length, zipLength));
        }
        ...
        {
            GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ResourceUpdateFailureGameEvent>().Fill(name, downloadUri, retryCount, totalRetryCount, errorMessage));
        }
    }
}
