...
namespace ZJY.Framework
{
    ...
    public class EntityComponent : GameBaseComponent, IUpdateComponent
    {
        ...
        private void OnShowEntitySuccess(EntityBase entity, float duration, object userData)
        {
            ...
            {
                GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ShowEntitySuccessGameEvent>().Fill(entity, duration, userData));
            }
        }


        private void OnShowEntityFailure(int entityId, string entityAssetName, string entityGroupName, Type entityLogicType, string errorMessage, object userData)
        {
            Log.Error(string.Format("Show entity failure, entity id '{0}', asset name '{1}', entity group name '{2}', error message '{3}'.", entityId.ToString(), entityAssetName, entityGroupName, errorMessage));
            if (m_EnableShowEntityFailureEvent)
            {
                GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ShowEntityFailureGameEvent>().Fill(entityId, entityAssetName, entityGroupName, entityLogicType, errorMessage, userData));
            }
        }

        private void OnShowEntityUpdate(int entityId, string entityAssetName, string entityGroupName, Type entityLogicType, float progress, object userData)
        {
            if (m_EnableShowEntityUpdateEvent)
            {
                GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ShowEntityUpdateGameEvent>().Fill(entityId, entityAssetName, entityGroupName, entityLogicType, progress, userData));
            }
        }

        private void OnShowEntityDependencyAsset(int entityId, string entityAssetName, string entityGroupName, Type entityLogicType, string dependencyAssetName, int loadedCount, int totalCount, object userData)
        {
            if (m_EnableShowEntityDependencyAssetEvent)
            {
                GameEntry.Event.CommonEvent.Dispatch(this, GameEntry.Pool.SpawnClassObject<ShowEntityDependencyAssetGameEvent>().Fill(entityId, entityAssetName, entityGroupName, entityLogicType, dependencyAssetName, loadedCount, totalCount, userData));
            }
        }
