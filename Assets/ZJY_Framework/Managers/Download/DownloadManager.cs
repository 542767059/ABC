...
namespace ZJY.Framework
{
    public class DownloadManager : ManagerBase
    {
        ...
        private void OnDownloadAgentUpdate(DownloadAgentBase downloadAgent, int lastDownloadedLength)
        {
            m_DownloadCounter.RecordDownloadedLength(lastDownloadedLength);
            if (DownloadUpdateEvent != null)
            {
                DownloadUpdateEvent(downloadAgent.Task.SerialId, downloadAgent.Task.DownloadPath, downloadAgent.Task.DownloadUri, downloadAgent.CurrentLength, downloadAgent.Task.UserData);
            }
        }
