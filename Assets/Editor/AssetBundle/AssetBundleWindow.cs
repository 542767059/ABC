...
public class AssetBundleWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("转换Lua脚本完毕", GUILayout.Width(200)))
        {
    ...
    private void CopyLuaDirectory(string sourceDirName, string destDirName)
    {
        try
        {
    ...
    private void OnAssetBundleCallBack()
    {
        ...
        //BuildAssetBundleOptions.UncompressedAssetBundle --不进行数据压缩如果使用该项，因为没有压缩\解压缩的过程， AssetBundle的发布和加载会很快，但是AssetBundle也会更大，下载更慢
        //BuildAssetBundleOptions.ChunkBasedCompression --AssetBundle的压缩格式为lz4.默认的是lzma格式，下载Assetbundle后立即解压而lz4格式的AssetBundle会在加载资源的时候进行解压，只是解压资源的时机不一样
