...
namespace UnityGameFramework.Editor.AssetBundleTools
{
    public sealed partial class AssetBundleBuilderController
    {
        ...
        private bool BuildAssetBundles(Platform platform, AssetBundleBuild[] buildMap, BuildAssetBundleOptions buildOptions)
        {
            ...
            ProcessManifest(platform, workingPath, outputPackagePath, outputFullPath, outputPackedPath, ZipSelected, platform.ToString(), null);
        ...
        private void ProcessManifest(Platform platform, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string assetBundleName, string assetBundleVariant)
        {
            string assetBundleFullName = GetAssetBundleFullName(assetBundleName, assetBundleVariant).ToLower();
            m_AssetBundleDatas[assetBundleFullName] = new AssetBundleData(assetBundleName.ToLower(), assetBundleVariant, AssetBundleLoadType.LoadFromFile, false);
            ProcessAssetBundle(platform, workingPath, outputPackagePath, outputFullPath, outputPackedPath, zip, assetBundleName, assetBundleVariant);
        }
        ...
        private void ProcessAssetBundle(Platform platform, string workingPath, string outputPackagePath, string outputFullPath, string outputPackedPath, bool zip, string assetBundleName, string assetBundleVariant)
        {
            ...
            byte[] hashBytes = VerifierUtil.GetCrc32(bytes);
            int hashCode = ConverterUtil.GetInt32(hashBytes);
            ...
            if (OutputFullSelected)
            {
                ...
                if (zip)
                {
                    ...
                    zipHashCode = ConverterUtil.GetInt32(VerifierUtil.GetCrc32(zipBytes));
        ...
        private void ProcessPackageList(string outputPackagePath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(packageListPath, FileMode.CreateNew, FileAccess.Write))
            {
                ...
                {
                    ms.WriteInt(m_AssetBundleCollection.AssetCount);
                    ms.WriteInt(m_AssetBundleCollection.AssetBundleCount +1);

                    foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
                    {
                        ms.WriteUTF8String(assetBundleData.Name);

                        AssetBundleCode assetBundleCode = assetBundleData.GetCode(platform);

                        ms.WriteInt(assetBundleCode.Length);
                        ms.WriteInt(assetBundleCode.HashCode);

                        string[] assetNames = assetBundleData.GetAssetNames();
                        ms.WriteInt(assetNames.Length);
                        foreach (string assetName in assetNames)
                        {
                            ms.WriteUTF8String(assetName);
                        }
                    }
                    buffer = ms.ToArray();
                }
        ...
        private VersionListData ProcessVersionList(string outputFullPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(versionListPath, FileMode.CreateNew, FileAccess.Write))
            {
                ...
                using (MMO_MemoryStream ms = new MMO_MemoryStream())
                {
                    ...
                    foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
                    {
                        ...
                        ms.WriteInt(assetBundleCode.Length);
                        ms.WriteInt(assetBundleCode.HashCode);
                        ms.WriteInt(assetBundleCode.ZipLength);
                        ms.WriteInt(assetBundleCode.ZipHashCode);
            ...
            hashBytes = VerifierUtil.GetCrc32(bytes);
            int zipHashCode = ConverterUtil.GetInt32(hashBytes);
            ...
            Debug.Log(versionListPathWithCrc32AndSuffix + ": " + length + ": " + hashCode + ": " + zipLength + ": " + zipHashCode);
        ...
        private void ProcessReadOnlyList(string outputPackedPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(readOnlyListPath, FileMode.CreateNew, FileAccess.Write))
            {
                ...
                using (MMO_MemoryStream ms = new MMO_MemoryStream())
                {
                    ms.WriteInt(packedAssetBundleDatas.Count);
