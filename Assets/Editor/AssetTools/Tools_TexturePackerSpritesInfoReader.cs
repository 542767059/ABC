...
/// </summary>
public class Tools_TexturePackerSpritesInfoReader
{
    [MenuItem("Assets/Tools/分割从TexturePacker中导出的图片为精灵")]
    ...
    private static void ChangeTextureFormatAccordingToFileInfo(Texture2D tex)
    {
        if (ReadTextureFile(tex))
        {
            ChangeTextureFormat(tex);
        }
    }
    ...
    /// </summary>
    /// <param name="tex"></param>
    private static void ChangeTextureFormat(Texture2D tex)
    {
        //首先获取图片的原始尺寸(不超过4096)
        ...
        {
            metaList.Add(sprites[i].ConvertToSpriteMetaData());
            count++;
        }
